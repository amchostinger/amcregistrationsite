...
export default function PaymentFailed({ paymentId, reference }) {
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      setStatus(data.status || (data.paid ? 'paid' : 'unknown'));
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-[320px] flex items-center justify-center p-6">
      <div className="w-full max-w-xl rounded-[28px] border border-[#e8e0d0] bg-white p-8 shadow-[0_20px_50px_rgba(15,30,51,0.08)]">
        <div className="flex flex-col items-center text-center gap-4">
          <div className="w-20 h-20 rounded-full flex items-center justify-center bg-[#fff1f2] border border-[#ef4444]/20">
          ...
          <p className="max-w-xl font-body text-sm text-gray-600">
            We could not confirm your payment at this time. This may be temporary — please verify the payment status below.
          </p>
          <div className="rounded-3xl border border-[#e8e0d0] bg-[#fef3f2] px-5 py-4 w-full text-left text-sm text-red-700">
